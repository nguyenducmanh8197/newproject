...
export const selectCategoryById = (id: string) =>
  createSelector([selectCategories], (categories: any[]) => categories.find((cat: any) => cat.id === id));
...
export const selectActiveCategories = createSelector(
  [selectCategories],
  (categories: any[]) => categories.filter((cat: any) => cat.isActive)
);
...
export const selectCategoriesByType = (type: number) =>
  createSelector(
    [selectCategories],
    (categories: any[]) => categories.filter((cat: any) => cat.type === type)
  );

/**
 * Select categories grouped by type
 */
export const selectCategoriesGroupedByType = createSelector([selectCategories], (categories: any[]) => {
